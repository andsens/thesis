...
In this chapter we will assess the usefulness of Comb in light of the demo
applications from chapter \ref{chap:demo}. Following this, we will analyze
whether we achieved the goals we set ourselves.
...
Ref to scenario in requirements
\todo{iterations with CSS ids suck, mustache comments solve that problem (instead of relying on weird css classes)}

\todo{Potential application: screen scrapers. Allow people to get data from a website without having to create a rest api}
...
This quality of Comb combined with the parentNode
...
If previous comb file exists, tell user if dataset structure has changed
\todo{Make a checker, to figure out if the retrieved dataset structure has changed}

\todo{Rule of Robustness: When templates change, transparency}
...
\todo{Rule of Extensibility: we are doing exactly that by cutting stuff away}
