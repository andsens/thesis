...
As we describe in section \ref{sec:standalone-lines} mustache removes
...
section has performed\footnote{
	not accounting for effects of the modification proposed in
	section \ref{sec:flatten-section}
...
The backbone library\footnote{See section \ref{sec:backbone}} features an event
...
Node.js\footnote{See section \ref{sec:nodejs}} allows execution of JavaScript on
the server-side. With node.js Comb could be used in the parsing of rendered
