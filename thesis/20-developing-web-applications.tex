...
\begin{figure}
	\centering
	\input{graphics/asynchronous}
	\caption{Diagram of synchronous communication between client and server}
	\label{fig:asynchronous}
\end{figure}

\subsection{\todo{Needs a title}}
...
A web application can incorporate business logic \todo{definition} and
...
Web applications can also wholly be made up of client-side code, leaving the
server to only supply static content. "Mashups" \todo{definition} are fitting
...
\begin{itemize}
	...
	a Content Delivery Network (CDN \todo{definition}).
	...
	\todo{Examples: Passpack, Strongcoin}
	...
	\todo{Examples: Google Chrome apps, Offline GMail with Google Gears}
\end{itemize}

\subsection{Combining the strengths}
These arguments do not make the case for either approach to construct a
web application. They instead highlight the strengths of both.
A combination of server-side and client-side processing where their respective
advantages are utilized and their drawbacks avoided, will in part help
creating a well developed web application.
