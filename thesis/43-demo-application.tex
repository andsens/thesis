...
\label{chap:demo}
This chapter describes the workings of two applications utilizing Comb.
The first application shows an actual use case of Comb and highlights its
strengths by comparison.
The second application demonstrates the internal workings of Comb.
...
as the plot and synopsis of a movie. We will implemented the same application
once again in much the same way\footnote{
	Although we will use a few more sophisticated tools,
	see section \ref{sec:mdb2-tools}
}.

We use chaplin\footnote{Section \ref{sec:chaplin}} to structure our application
on the client side, while a PHP REST framework together with
php-activerecord\footnote{Section \ref{sec:php-activerecord}} function
as a communication layer to the MySQL database for the client.
...
In both implementations, Movies can be browsed, created and edited.
...
also initializing subviews that belong to it.
...
it should manager in the interface.
...
database. In figure \ref{fig:movie-jquery-no-bootstrap} we retrieve all the
...
@model.set 'plot', @$('details.plot p').text()
	\end{lstlisting}
	\caption{Populating a Movie Model with values retrieved with jQuery}
	\label{fig:movie-jquery-no-bootstrap}
\end{figure}

Figure \ref{fig:movie-comb-no-bootstrap} shows the same procedure, only here we
use Comb to retrieve those values. We can clearly see that there are no
references to the structure of the template used.
\begin{figure}
	\centering
	\begin{lstlisting}
...
@model.set 'plot', @data.plot.value
	\end{lstlisting}
	\caption{Populating a Movie Model with values retrieved with Comb}
	\label{fig:movie-comb-no-bootstrap}
\end{figure}
...
retrieving values from the DOM. The code in figure \ref{fig:movie-comb-no-bootstrap}
and \ref{fig:movie-comb-bootstrap} is exactly the same.
\begin{figure}
	\centering
	\begin{lstlisting}
@model.set 'id', @data.id.value
...
functions.
It also serves as a tool for testing the tool itself.
...
The template is then rendered\footnote{
	We use mustache.js to render the template directly in the browser.
}
...
the template has been rendered. This form can be used to control all aspects of
the dataset that is fed into the template rendering engine.

As such, the application is rather uninteresting. The more interesting part is
