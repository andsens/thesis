...
constrained by the environment the actual web server runs in. The running time
of our tool +will only a minor concern.
...
\item Whether an identifier refers to a key in the current stack level or
      a to a key in one of the lower stack levels.
\end{itemize}
...
\item Use require-js to load templates and their information in parallel
      while developing and inline both template and template information
      into one big JavaScript file when deploying.
\end{itemize}
...
and it obsoletes the pre-parser.
...
in chapter \ref{chap:requirements} illustrates this point very well},
